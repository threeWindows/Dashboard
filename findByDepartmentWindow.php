...
<?php
  ...
  <div id='mainContainer'>
    ...
    $selectEmployeeData = mysqli_prepare($conn, "SELECT id_pracownika, imie_p, nazwisko_p, telefon_p, email_p, id_oddzialu FROM pracownik WHERE id_oddzialu = $index");
      ...
      while(mysqli_stmt_fetch($selectEmployeeData)) {
        echo"
          <form action='findByEmpWindow.php' method='post' class='deviceElement'>
            ...
            <ul>
              <div style='display: none;'>
                <input type='text' name='constIndexDev' value='empId'>
              </div>
              <li>Imię: <span i='empId' class='serialNumber'>$empName</span><input type='text' name='changeSerialNumber' class='inp' style='background-color: rgb(230,230,230);' i='empId'></li>
              <li>Nazwisko: <span i='empId'>$empLastName</span><input type='text' name='changeProducent' class='inp' i='empId' style='background-color: rgb(230,230,230);'></li>
              <li>Numer Telefonu: <span i='empId'>$empTelephone</span><input type='text' name='changeModel' class='inp' i='empId' style='background-color: rgb(230,230,230);'></li>
              <li>Adres Email: <span i='empId'>$empEmail</span><input type='text' name='changeCategory' class='inp' i='empId' style='background-color: rgb(230,230,230);'></li>
            </ul>
            <input type='button' value='Aktualizuj Dane' class='updateData' i='empId' empId='$empId'>
            <input type='submit' value='Zatwierdź' class='confirm' name='confirm' i='empId'>
          </form>";
      }
    echo "</div>";


    $selectDeviceData = mysqli_prepare($conn, "SELECT sprzet.id_sprzetu, sprzet.nr_seryjny, sprzet.producent, sprzet.model, sprzet.kategoria, pracownik.id_pracownika, pracownik.id_oddzialu, oddzialy.id_oddzialu, zgloszenia.id_pracownika, zgloszenia.id_sprzetu FROM pracownik INNER JOIN oddzialy ON pracownik.id_oddzialu = oddzialy.id_oddzialu INNER JOIN zgloszenia ON zgloszenia.id_pracownika = pracownik.id_pracownika INNER JOIN sprzet ON zgloszenia.id_sprzetu = sprzet.id_sprzetu WHERE oddzialy.id_oddzialu = $index;");
...
<script src='js/findByDepWindow.js'></script>